...
def create_arg_parser():
    ...
    parser.add_argument("-l", "--limit_train", default=0, type=int,
                        help="Limit train set to this amount of insts (default 0 means no limit)")
...
def print_best_features(vectorizer, clf, X_train, only_words):
    ...
    if only_words:
        if not os.path.isfile(word_list_file):
            raise ValueError("First download the word-list file from https://github.com/dwyl/english-words and save it in data/english_words/words_dictionary.json")
...
def read_test_data(test_file, filter_categories):
    ...
    # Filter categories if we want
    if has_labels and filter_categories:
...
def main():
    ...
    X_train, Y_train = get_data(args.input_file, args.filter_categories, args.down_sample,
                                args.also_other, args.limit_train)

    # Read in dev/test set if we specified it
    if args.test_file:
        X_test, Y_test = read_test_data(args.test_file, args.filter_categories)
    ...
    if args.test_file:
        ...
        if args.test_no_labels:
            if args.probabilities:
                ...
                out_lines = [" ".join(clf.named_steps['cls'].classes_)] + \
                            [" ".join([str(x) for x in row]) for row in probs]
            else:
    ...
    if args.confusion:
        ...
        plot_confusion_matrix(confusion_matrix(Y_plot, Y_pred),
                              [to_small[c] for c in clf.named_steps['cls'].classes_],
                              args.confusion, normalize=False)
