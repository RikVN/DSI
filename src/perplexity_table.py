...
def get_ppl_from_file(in_file, oov_type):
    ...
    for line in open(in_file, 'r'):
        if line.strip().startswith("Perplexity {0} OOVs:".format(oov_type)):
            val = str(int(round(float(line.strip().split()[-1]), 0)))
            return val
...
def get_closest_dsis(ppl_list, exs_dsis):
    ...
    for idx, values in enumerate(ppl_list):
        ...
        pairs = sorted([[value, dsi] for value, dsi in zip([int(x) for x in values[1:]], sml_dsis)], key=lambda x: x[0])
        # Convert to nice format for printing
        out_list.append([small_dsis[exs_dsis[idx]]] + ["{0} ({1})".format(dsi, value) for value, dsi in pairs])
    return out_list
